...
namespace Bottom_API._Services.Services
{
    public class ReceivingService : IReceivingService
    {
        ...
        public async Task<List<MaterialEditModel>> EditMaterial(ReceiveNoMain model)
        {
            ...
            dataList1 = dataList1.OrderBy(x => decimal.Parse(x.Order_Size)).ToList();
            return dataList1;
        }
        ...
        public async Task<bool> EditDetail(List<MaterialEditModel> data)
        {
            ...
            if (missing_No == "")
            {
                ...
                //------------------------- Update giá trị Status--------------------------------------------//
                await this.UpdateStatusMaterial(data[0].Purchase_No, data[0].MO_No, data[0].MO_Seq_Edit, missing_No);
        ...
        public async Task<bool> UpdateStatusMaterial(string purchaseNo,string mo_No, string mOSeq, string missingNo)
        {
            if (missingNo == "")
            {
                var checkStatus = "Y";
        ...
        public async Task<bool> UpdateStatusMaterialSubmit(string purchaseNo, string mo_No, string mOSeq, string missingNo)
        {
            if (missingNo == "")
            {
                var checkStatus = "Y";
