...
namespace Bottom_API._Services.Services
{
    public class TransferFormService : ITransferFormService
    {
        ...
        public List<Transfer_Form_Print_Qty_Dto> GetTransferFormPrintQty(string collectTransNo)
        {
            ...
            if (transactionDetailResultAct.Any())
            {
                data = transactionDetailResult.Join(
                        ...
                        (x, y) => new { x, y })
                            .Join(
                                transactionDetailResultActTranQty,
                                x => new { Tool_Size = x.x.Tool_Size.Trim(), Order_Size = x.x.Order_Size.Trim() },
                                y => new { Tool_Size = y.Tool_Size.Trim(), Order_Size = y.Order_Size.Trim() },
                                (x, y) => new { x, y })
                                    .Join(
                                        transactionDetailResultAct,
                                        x => new { Tool_Size = x.x.x.Tool_Size.Trim(), Order_Size = x.x.x.Order_Size.Trim() },
                                        y => new { Tool_Size = y.Tool_Size.Trim(), Order_Size = y.Order_Size.Trim() },
                                        (x, y) => new Transfer_Form_Print_Qty_Dto
                                        {
                                            Tool_Size = x.x.x.Tool_Size,
                                            Order_Size = x.x.x.Order_Size,
                                            Trans_Qty = x.x.x.Trans_Qty,
                                            MO_Qty = x.x.y.MO_Qty,
                                            Act_Trans_Qty = x.y.Trans_Qty,
                                            Act_Qty = y.Trans_Qty
        ...
        {
            var transactionMain = _transactionMainRepo.FindAll(x => x.Is_Transfer_Form == "N" && x.Transac_Type != "O" && x.Transac_Type != "R" && x.Can_Move == "Y");
            var qrCodeMain = _qRCodeMainRepository.FindAll();
            var packingList = _packingListRepository.FindAll();
                        ...
                        .GroupJoin(
                            ...
                            .SelectMany(
                                ...
                                (x, y) => new Transfer_Form_Generate_Dto
                                {
                                    ...
                                    Subcon_ID = y.Subcon_ID,
                                    Subcon_Name = y.Subcon_Name,
                                    Purchase_Qty = _packingListDetailRepository.SumPurchaseQtyByReceiveNo(y.Receive_No),
                                    Transacted_Qty = x.TransactionMainQrCodeMain.TransactionMain.Transacted_Qty,
                                    Factory_ID = _materialPurchaseRepository.GetFactoryID(y.Purchase_No, y.MO_No, y.MO_Seq, y.Material_ID)
                                });
            if (isSubcont == "N")
            {
        ...
        public async Task<bool> ReleaseTransferForm(List<Transfer_Form_Generate_Dto> generateTransferForm, string updateBy)
        {
            ...
            {
                var transferFormList = _transferFormRepository.FindAll(x => x.Collect_Trans_No.Trim() == item.Collect_Trans_No.Trim()).ToList();
        ...
        public List<dynamic> GetAccumulatedTransQtyReleaseTransferForm(string collectTransNo)
        {
            ...
            var transferFormAct = _transferFormRepository.FindAll(x => x.Collect_Trans_No == collectTransNo).FirstOrDefault();
            var transacNoAct = _transferFormRepository.FindAll(x => x.MO_No == transferFormAct.MO_No && x.MO_Seq == transferFormAct.MO_Seq && x.Material_ID == transferFormAct.Material_ID && x.Is_Release == "Y").Select(x => x.Transac_No).ToList();
        ...
        {
            List<dynamic> transactionDetailTotal = new List<dynamic>();
            var transacNo = await _transferFormRepository.FindAll(x => x.Collect_Trans_No == collectTransNo).Select(x => x.Transac_No).ToListAsync();
            foreach (var itemTransacNo in transacNo)
            {
