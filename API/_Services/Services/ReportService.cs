...
namespace Bottom_API._Services.Services
{
    public class ReportService : IReportService
    {
        ...
        {
            var data = await (_context.ReportMatRecExcel_Dto.FromSqlRaw("EXEC [dbo].[NSP_Receive_Material_Report] @DateType, @Date_S, @Date_E, @MO_No, @MO_Seq, @Supplier, @Status ,@Article",
            ...
            new SqlParameter("Article", MaterialReceiveParam.Article)
                )).ToListAsync();
            data.ForEach(item => {
                if(item.Order_Status == "Y") {
                    item.Order_Status = "Y.Close";
                } else if(item.Order_Status == "D") {
                    item.Order_Status = "D.Delete";
