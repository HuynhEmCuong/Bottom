    ...
    public class KanbanByPoService : IKanBanByPoService
    {
        ...
        public async Task<List<KanbanByPoDetail_Dto>> GetKanbanByPoDetail(string moNo, string moSeq)
        {
            ...
            foreach (var item in dataGroup)
            {
                ...
                kanban.Prs = data.Where(x => x.Material_Name.Trim() == item.Key.Material_Name.Trim() && x.Title_ID == "Prs").FirstOrDefault().Total_Pairs;
                kanban.RevQty = data.Where(x => x.Material_Name.Trim() == item.Key.Material_Name.Trim() && x.Title_ID == "Rev_Qty").FirstOrDefault().Total_Pairs;
                kanban.StkQty = data.Where(x => x.Material_Name.Trim() == item.Key.Material_Name.Trim() && x.Title_ID == "Stk_Qty").FirstOrDefault().Total_Pairs;
                listData.Add(kanban);
            }
