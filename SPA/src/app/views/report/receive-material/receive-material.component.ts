...
export class ReceiveMaterialComponent implements OnInit {
  ...
  exportExcel() {
    ...
    this.reportService.getMaterialReceive(this.dateType, d1, d2, this.moNo, this.batch,
    this.supplier, this.status,this.article).subscribe((res) => {
      if(res.length === 0) {
        this.spiner.hide();
        this.alertifyService.error("NO DATA!!!");
      } else {
        this.reportService.exportExcelMaterialReceive(this.dateType, d1, d2, this.moNo, this.batch,this.supplier, this.status,this.article);
      }
    });
  }
