...
export class InputMainComponent implements OnInit {
  ...
  getInputMain(e) {
    ...
    if (this.rackLocation === "") {
      ...
      if (this.functionUtility.checkQrCodeOfLength(e)) {
        let flag = true;
        ...
        if (flag) {
          ...
          this.qrCodeIdCurrent = e.toString().trim();
  ...
  async submitInput() {
    ...
    for (const element of this.result) {
      element.input_No = await this.functionUtility.getInputNo(element.plan_No);
    }
    this.result.forEach((e, i) => {
      if (e.input_No == null)
        this.err = false;
